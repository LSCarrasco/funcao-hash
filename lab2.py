...
print("Frase - SHA256 - MD5")

for frase_hash in frases:
    frase = frase_hash["frase"]
    sha256 = frase_hash["sha256"]
    md5 = frase_hash["md5"]
    
    SHA256 = hashlib.sha256(frase.encode('utf-8')).hexdigest()
    MD5 = hashlib.md5(frase.encode('utf-8')).hexdigest()    

    ver_sha256 = "Verdadeiro" if sha256 == SHA256 else "Falso"
    ver_md5 = "Verdadeiro" if md5 == MD5 else "Falso"

    print(f"{frase} - {ver_sha256} - {ver_md5}")
